#!/usr/bin/env python3
"""Walk-Forward Parameter Grid Search with BH/FDR Correction.

Extends backtest_frozen_position_rules.py with itertools.product grid search
to evaluate multiple parameter combinations systematically.
...
Usage:
  ...
  python scripts/walk_forward_param_grid.py --quick  # Reduced grid for testing
"""
...
class GridParams:
    ...
    recent_draws: int
    recent_weight: float
    train_min_support: int
    val_min_support: int
    exclude_lb_train: float
    include_lb_train: float
    exclude_lb_val: float
    include_lb_val: float
    exclude_max: int
    include_max: int
    hard_exclude_lb: float
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Walk-Forward Parameter Grid Search with BH/FDR")
    ...
    p.add_argument("--output", default="results/walk_forward_param_grid.json")
    return p.parse_args()


def _build_param_grid(quick: bool = False) -> list[GridParams]:
    """Build parameter grid for search.

    Full grid: ~100 combinations
    Quick grid: ~12 combinations for testing
    """
    if quick:
        grid = {
            "recent_draws": [365],
            "recent_weight": [0.5, 0.7],
            "train_min_support": [10],
            "val_min_support": [10],
            "exclude_lb_train": [0.78, 0.82],
            "include_lb_train": [0.30, 0.35],
            "exclude_lb_val": [0.76, 0.80],
            "include_lb_val": [0.28, 0.32],
            "exclude_max": [3],
            "include_max": [5],
        ...
        grid = {
            "recent_draws": [180, 365, 730],
            "recent_weight": [0.4, 0.6, 0.8],
            "train_min_support": [8, 10, 15],
            "val_min_support": [8, 10, 15],
            "exclude_lb_train": [0.75, 0.80, 0.85],
            "include_lb_train": [0.28, 0.33, 0.38],
            "exclude_lb_val": [0.73, 0.78, 0.83],
            "include_lb_val": [0.26, 0.31, 0.36],
            "exclude_max": [2, 3, 5],
            "include_max": [3, 5, 7],
            "hard_exclude_lb": [0.90, 0.95],
            "exclude_weight": [1.5, 2.0, 2.5],
            "include_weight": [0.8, 1.0, 1.2],
        }

    # Sample strategically to keep grid size manageable (~100 combos)
    # Use Latin Hypercube-like sampling by fixing some params while varying others
    params_list = []

    if quick:
        # Full cartesian for quick mode
        keys = list(grid.keys())
        for combo in product(*[grid[k] for k in keys]):
            params_list.append(GridParams(**dict(zip(keys, combo))))
    else:
        # Strategic sampling for full mode to get ~100 combinations
        # Core variation: recent_weight x exclude_lb_train x include_lb_train x exclude_max
        core_combos = list(product(
            grid["recent_weight"],
            grid["exclude_lb_train"],
            grid["include_lb_train"],
            grid["exclude_max"],
        ))

        # For each core combo, sample from remaining params
        for rw, ex_lb, in_lb, ex_max in core_combos:
            # Pick representative values for other params
            params_list.append(GridParams(
                recent_draws=365,
                recent_weight=rw,
                train_min_support=10,
                val_min_support=10,
                exclude_lb_train=ex_lb,
                include_lb_train=in_lb,
                exclude_lb_val=ex_lb - 0.02,  # Slightly relaxed for validation
                include_lb_val=in_lb - 0.02,
                exclude_max=ex_max,
                include_max=5,
                hard_exclude_lb=0.95,
                exclude_weight=2.0,
        ...
        # Add some variations of secondary params
        for rd in [180, 730]:
            params_list.append(GridParams(
                recent_draws=rd,
                recent_weight=0.6,
...
def main() -> int:
    ...
    param_grid = _build_param_grid(quick=args.quick)
    keno_types = [int(t) for t in args.types]

    print(f"Walk-Forward Parameter Grid Search")
    ...
    for idx, params in enumerate(param_grid):
        print(f"[{idx + 1}/{len(param_grid)}] Running params: recent_weight={params.recent_weight}, "
              f"exclude_lb_train={params.exclude_lb_train}, include_lb_train={params.include_lb_train}")

        result = _run_single_param_combo(
            draws,
            train_end=train_end,
            val_end=val_end,
    ...
    # Structure: (param_idx, keno_type, window) -> p-value
    pvalue_list: list[tuple[int, int, str, Optional[float]]] = []
    for param_idx, res in enumerate(results):
        for typ_key, typ_data in res["by_type"].items():
            keno_type = int(typ_key.split("_")[1])
            # Overall p-value
            pvalue_list.append((param_idx, keno_type, "overall", typ_data.get("big_win_pvalue")))
    ...
    for i, (param_idx, keno_type, window, _) in enumerate(pvalue_list):
        significance_map[(param_idx, keno_type, window)] = significant_flags[i]
    ...
    for param_idx, res in enumerate(results):
        for typ_key, typ_data in res["by_type"].items():
            keno_type = int(typ_key.split("_")[1])
            is_sig = significance_map.get((param_idx, keno_type, "overall"), False)
            typ_data["fdr_significant"] = is_sig
            if is_sig:
                n_significant += 1

    # Find best configurations per type
    best_by_type: dict[int, dict[str, Any]] = {}
    for keno_type in keno_types:
        ...
        if best_result:
            best_by_type[keno_type] = best_result
    ...
    payload = {
        "analysis": "walk_forward_param_grid",
        ...
        "best_by_type": {f"typ_{k}": v for k, v in best_by_type.items()},
    ...
    print("\n" + "=" * 80)
    print(f"Grid Search Complete: {len(param_grid)} combinations, {elapsed:.1f}s")
    print(f"FDR Correction: {n_significant}/{total_comparisons} significant at alpha={args.fdr_alpha}")
    print("=" * 80)

    print("\nBest configurations per type:")
    for keno_type in sorted(best_by_type.keys()):
        b = best_by_type[keno_type]
        sig_marker = "*" if b["fdr_significant"] else ""
        print(f"  Typ-{keno_type}: delta_roi={b['delta_roi']:+.2f}pp{sig_marker} "
