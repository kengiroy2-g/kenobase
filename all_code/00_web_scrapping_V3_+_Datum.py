...
def main(url, output_path):
    ...
    all_data = []
    start_date = "Di, 20.02."  # Festgelegtes Startdatum

    try:
        driver.get(url)
        select_year(driver, '2024')  # Wählen Sie das Jahr 2023 aus dem Dropdown
        ...
        for value, date_text in dates:
            ...
            if start_extraction:
                for keno_type in [str(i) for i in range(2, 11)]:  # Keno-Typen 2 bis 10
                    def operation():
                        date_select = Select(WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, 'gcid'))))
...
if __name__ == "__main__":
    ...
    output_path = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\keno_RohData_02_2024.csv"
