...
def generiere_kombinationen(zahlenpool, ziel_summe):
    return [
        kombi for kombi in combinations(zahlenpool, 6)
...
def zaehle_kombination_batch(kombinationen, archiv_df):
    ergebnisse = []
    standard_datum = datetime(2004, 1, 1)  # Standarddatum für nicht erschienene Kombinationen
    for kombi in kombinationen:
        zählung = {zahl: 0 for zahl in kombi}
        durchgelaufene_ziehungen = 0
        kombi_datum = standard_datum  # Setzen des Datums auf das Standarddatum

        for index, row in archiv_df.iterrows():
            gezogene_zahlen = set(row[1:21].dropna().astype(int))
            ziehungsdatum = row['Datum']
            durchgelaufene_ziehungen += 1

            if set(kombi).issubset(gezogene_zahlen):
                if kombi_datum == standard_datum or ziehungsdatum < kombi_datum:
                    kombi_datum = ziehungsdatum  # Aktualisieren des Datums, wenn die Kombination erschienen ist
        ...
        ergebnisse.append((kombi, durchgelaufene_ziehungen, zählung, counter_string, kombi_datum.strftime('%d.%m.%Y')))
    return ergebnisse
...
def zaehle_kombinationen(kombinationen, archiv_df):
    ...
    ergebnisse_df = pd.DataFrame(ergebnisse, columns=['Kombination', 'Anzahl der durchgelaufenen Ziehungen', 'Zählungen', 'Counter', 'Datum'])
