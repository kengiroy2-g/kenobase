    ...
    for kombi in kombinationen:
        ...
        for index, row in archiv_df.iterrows():
            ...
            if aktuelle_uebereinstimmung == kombi_set and kombi_datum is None:
                ...
                break  # Stoppe die Suche, sobald die Kombination gefunden wurde

        # Schritt 2: Zählungen und Ermittlung von Duos, Trios und Quatros
        for index, row in archiv_df.iterrows():
            if durchgelaufene_ziehungen >= index:  # Nur bis zur gefundenen Ziehung zählen
                gezogene_zahlen = set(row[1:21].dropna().astype(int))
...
def zaehle_kombinationen(kombinationen, archiv_df):
    ...
    ergebnisse_df.to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\00_Keno_6-Kombi_Analyse\\ergebnisse_v6-3.csv", index=False)
...
def hauptfunktion_optimiert(archiv_pfad, ziel_summe, start_datum, end_datum, start_datum_pool, end_datum_pool):
    ...
    zahlenpool = generiere_zahlenpool_optimiert(gefiltertes_archiv_df, start_datum_pool, end_datum_pool)
