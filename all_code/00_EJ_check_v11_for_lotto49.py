...
def generiere_oder_lese_kombinationen(datei_pfad):
    #ziel_summen = { 149}
    selbstdefinierte_zahlen_menge = {5	,3	,17	,18	,11	,7	,34,9}##############################################################################################
    ...
    if os.path.exists(datei_pfad):
        ...
        kombinationen_df = pd.DataFrame([kombi for kombi in tqdm(alle_kombinationen, desc="Generiere Kombinationen") ]) #if sum(kombi) in ziel_summen  /if 3 in kombi and 19 in kombi
...
def check_keno_parallel(kombinationen, keno_data):
    ...
    for kombination in kombinationen:
        ...
        for index, row in keno_data.iterrows():
            ...
            if len(matches) >= 6:     ######################################################################### Werte immer anpassen
...
def vorpruefung_gegen_archiv_parallel(kombinationen_df, archiv_pfad, ergebnis_datei_pfad, checkpoint_pfad, batch_size=1):
    ...
    archiv_kombinationen = [set(row.dropna().astype(int)) for _, row in archiv_df.iterrows()]
...
def hauptprogramm():
    ...
    kombinationen_datei_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_kombinationen_lotto.csv"
    ...
    vorpruefung_datei_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_vorpruefung_ergebnisse_lotto.csv"
    vorpruefung_checkpoint_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_vorpruefung_checkpoint_lotto.csv"
    keno_checkpoint_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_keno_checkpoint_lotto.csv"
    ...
    pd.DataFrame(kombinationen_nach_archiv_df).to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_Zahlen_treffer_Detail.csv", index=False)
