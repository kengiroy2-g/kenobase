...
def zaehle_kombination_batch(kombinationen, archiv_liste):
    ...
    for kombi in kombinationen:
        ...
        # Verwende das standard_datum, wenn kein Erscheinungsdatum gefunden wurde
        if not kombi_datum:
            kombi_datum = standard_datum

        # Berechnungen von Zählung und Gruppierungen durchführen
        for datum, gezogene_zahlen in archiv_liste :   #if kombi_datum == standard_datum else reversed(archiv_liste)
            ...
            # Priorisiere Quatros, dann Trios, dann Duos
            if len(aktuelle_uebereinstimmung) == 4:
                quatro.update(combinations(aktuelle_uebereinstimmung, 4))
            elif len(aktuelle_uebereinstimmung) == 3 and not quatro:
                trios.update(combinations(aktuelle_uebereinstimmung, 3))
            elif len(aktuelle_uebereinstimmung) == 2 and not quatro and not trios:
                duos.update(combinations(aktuelle_uebereinstimmung, 2))

            if all(zählung[zahl] > 0 for zahl in kombi):
                break
        ...
        ergebnisse.append((
            kombi, 
            kombi_datum.strftime('%d.%m.%Y') if kombi_datum else standard_datum.strftime('%d.%m.%Y'), 
...
def zaehle_kombinationen(kombinationen, archiv_df):
    ergebnisse = []
    batch_size = 10  # Anpassbar basierend auf der Leistung
    ...
    ergebnisse_df.to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\00_Keno_6-Kombi_Analyse\\ergebnisse_v6-13.csv", sep=';', encoding='utf-8',index=False)
