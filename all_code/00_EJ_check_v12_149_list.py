...
#geänderte Logik bei Zahlengeneration und vorpruefung V2
#############################
...
def generiere_oder_lese_kombinationen(datei_pfad):
    ...
    selbstdefinierte_zahlen_menge =  {3, 6, 9, 10, 14, 17, 21, 24, 26, 29, 30, 36, 46, 48}  ##############################################################################################
    
    #{1, 3, 6, 9, 10, 12, 14, 17, 18, 20, 21, 24, 25, 26, 29, 30, 33, 34, 36, 38, 39, 40, 42, 44, 45, 48, 46}
    #{3,4,5,6,7,9,10,14,15,17,19,21,24,26,29,30,36,37,43,46,47,48}
    ...
    alle_kombinationen = list(combinations(selbstdefinierte_zahlen_menge, 5))
    kombinationen_df = pd.DataFrame([kombi for kombi in tqdm(alle_kombinationen, desc="Generiere Kombinationen") if sum(kombi) in ziel_summen ]) #if sum(kombi) in ziel_summen
...
def batch_vorpruefung(kombinationen_batch, archiv_kombinationen):
    ...
    for kombi in kombinationen_batch:
        if all(len(set(kombi) & archiv_kombi) <= 4 for archiv_kombi in archiv_kombinationen):
...
def check_keno_parallel(kombinationen, keno_data):
    ...
    for kombination in kombinationen:
        ...
        for index, row in keno_data.iterrows():
            ...
            if len(matches) >= 5:     ######################################################################### Werte immer anpassen
...
def vorpruefung_gegen_archiv_parallel(kombinationen_df, archiv_pfad, ergebnis_datei_pfad, checkpoint_pfad, batch_size=50):
...
def hauptprogramm():
    ...
    kombinationen_datei_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_kombinationen_149_list.csv"
    archiv_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\EJ_archiv_bereinigt_+_.csv"
    keno_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\KENO_2021.csv"
    vorpruefung_datei_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_vorpruefung_ergebnisse_149_list.csv"
    vorpruefung_checkpoint_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_vorpruefung_checkpoint_149_list.csv"
    keno_checkpoint_pfad = "C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_keno_checkpoint_149_list.csv"
    ...
    pd.DataFrame(treffer_details).to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys_Zahlen_treffer_Detail_149_list.csv", index=False)
    #pd.DataFrame(kombinationen_nach_archiv_df).to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\EJ_Datei\\sys22_Zahlen_treffer_Detail.csv", index=False)
