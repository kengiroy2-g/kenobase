...
def zaehle_kombination_batch(kombinationen, archiv_df):
    ...
    for kombi in kombinationen:
        ...
        # Konvertiere die Zeilen in eine Liste, um sie umkehrbar zu machen
        zeilen_liste = list(archiv_df.iterrows())

        # Schritt 1: Ermittle das erste Erscheinungsdatum der gesamten Kombination
        for index, row in reversed(zeilen_liste):
            gezogene_zahlen = set(row[1:21].dropna().astype(int))
            kombi_set = set(kombi)
            aktuelle_uebereinstimmung = kombi_set.intersection(gezogene_zahlen)

            if aktuelle_uebereinstimmung == kombi_set and kombi_datum is None:
                kombi_datum = row['Datum']
                break

        # Schritt 2: Beginne die Zählung und Gruppenbildung ab dem gefundenen Datum oder von Ende an
        for index, row in reversed(zeilen_liste):
            durchgelaufene_ziehungen += 1  # Ziehungszähler erhöhen
            if kombi_datum is not None and row['Datum'] > kombi_datum:
                continue
            ...
            if all(zahl > 0 for zahl in zählung.values()):
                break  # Stoppe, wenn alle Zahlen mindestens einmal erschienen sind

        # Wenn die Kombination nicht gefunden wurde, Standarddatum verwenden
        if kombi_datum is None:
...
def zaehle_kombinationen(kombinationen, archiv_df):
    ...
    ergebnisse_df = pd.DataFrame(ergebnisse, columns=['Kombination', 'Datum','Anzahl der durchgelaufenen Ziehungen', 'Zählungen', 'Counter', 'Duos', 'Trios', 'Quatros'])
    ergebnisse_df.to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\00_Keno_6-Kombi_Analyse\\ergebnisse_v6-4.csv", index=False)
