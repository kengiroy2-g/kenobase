...
# Duos,trios,quatros werden falsch berechnet ,korrektur in v7
...
def zaehle_kombination_batch(kombinationen, archiv_liste):
    ...
    for kombi in kombinationen:
        ...
        # Ermitteln des ersten Erscheinungsdatums der Kombination
        for datum, gezogene_zahlen in reversed(archiv_liste):
            if set(kombi) <= gezogene_zahlen:
                kombi_datum = pd.to_datetime(datum)
                break

        # Wenn die Kombination nie vollständig erschienen ist, vom neuesten Datum starten
        if not kombi_datum:
            kombi_datum = standard_datum

        # Zählung der Erscheinungen nach dem Erscheinungsdatum oder vom neuesten Datum
        for datum, gezogene_zahlen in (archiv_liste if kombi_datum == standard_datum else reversed(archiv_liste)):
            ...
            # Aktualisieren von Duos, Trios und Quatros
            for combo in combinations(aktuelle_uebereinstimmung, 2):
                duos.add(combo)
            for combo in combinations(aktuelle_uebereinstimmung, 3):
                trios.add(combo)
            for combo in combinations(aktuelle_uebereinstimmung, 4):
                quatro.add(combo)

            if all(count > 0 for count in zählung.values()):
                break

        # Ergebnisse formatieren
        counter_string = ','.join(str(zählung[zahl]) for zahl in kombi)
        duos_string = ';'.join(map(str, duos))
        trios_string = ';'.join(map(str, trios)) if trios else 'Keine Trios gefunden'
        quatro_string = ';'.join(map(str, quatro)) if quatro else 'Keine Quartos gefunden'

        ergebnisse.append((
            kombi, 
            kombi_datum.strftime('%d.%m.%Y'), 
            durchgelaufene_ziehungen, 
            zählung, 
            counter_string, 
            duos_string, 
            trios_string, 
    ...
    #     ergebnisse.append((kombi, kombi_datum.strftime('%d.%m.%Y') if kombi_datum else standard_datum.strftime('%d.%m.%Y'), durchgelaufene_ziehungen, zählung, counter_string, duos_string, trios_string, quatro_string))
...
def zaehle_kombinationen(kombinationen, archiv_df):
    ...
    ergebnisse_df.to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\00_Keno_6-Kombi_Analyse\\ergebnisse_v6-10.csv", index=False)
...
def hauptfunktion_optimiert(archiv_pfad, ziel_summe, start_datum, end_datum, start_datum_pool, end_datum_pool):
    ...
    zahlenpool = {3, 6, 9, 10, 17, 21, 26, 29, 30, 46, 48}  #generiere_zahlenpool_optimiert(gefiltertes_archiv_df, start_datum_pool, end_datum_pool)
...
if __name__ == '__main__':
    ...
    start_datum = pd.to_datetime('2022-02-02')
