...
#####################
#v9 Swischen speichern der Daten
...
def zaehle_kombinationen(kombinationen, archiv_df):
    ...
    batch_index = 0  # Führen Sie einen Index für jede Batch-Verarbeitung ein
    ...
    with ProcessPoolExecutor(max_workers=5) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc='Zähl-Verarbeitung'):
            try:
                batch_ergebnisse = future.result()
                ergebnisse.extend(batch_ergebnisse)
                # Speichern Sie die Ergebnisse nach jeder Batch-Verarbeitung
                batch_ergebnisse_df = pd.DataFrame(batch_ergebnisse, columns=['Kombination', 'Datum','Anzahl Ziehungen', 'Counter_Index','Index', 'Duos', 'Trios', 'Quatros'])
                batch_ergebnisse_df.to_csv(f"C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\00_Keno_6-Kombi_Analyse\\zwischenergebnisse_batch_{batch_index}.csv", sep=';', encoding='utf-8', index=False, mode='a', header=not os.path.exists(f"zwischenergebnisse_batch_{batch_index}.csv"))
                batch_index += 1
            except Exception as e:
                print(f"Ein Fehler ist aufgetreten: {e}")

    # Speichern Sie am Ende alle Ergebnisse in einer Datei
    ergebnisse_df = pd.DataFrame(ergebnisse, columns=['Kombination', 'Datum','Anzahl Ziehungen', 'Counter_Index','Index', 'Duos', 'Trios', 'Quatros'])
    ergebnisse_df.to_csv("C:\\Users\\Admin1\\Documents\\01_PROJECT\\Keno_GPTs\\Kenogpts_2\\00_Keno_6-Kombi_Analyse\\ergebnisse_v6-21.csv", sep=';', encoding='utf-8', index=False)
...
if __name__ == '__main__':
    ...
    end_datum = pd.to_datetime('2024-03-24')
    start_datum_pool = pd.to_datetime('2024-03-01')
    end_datum_pool= pd.to_datetime('2024-03-24')
