...
def generiere_oder_lese_kombinationen(datei_pfad):
    if os.path.exists(datei_pfad):
        ...
        kombinationen_df = pd.DataFrame([kombi for kombi in tqdm(alle_kombinationen, desc="Generiere Kombinationen") if len([z for z in kombi if z <= 12]) >= 2])
...
def vorpruefung_gegen_archiv_parallel(kombinationen_df, archiv_pfad, ergebnis_datei_pfad, batch_size=1000):
    if os.path.exists(ergebnis_datei_pfad):
        ...
        kombinationen_batches = [kombinationen[i:i+batch_size] for i in range(0, len(kombinationen), batch_size)]
        
        ergebnisse = []
        with ProcessPoolExecutor(max_workers=15) as executor:
            futures = [executor.submit(batch_vorpruefung, batch, archiv_kombinationen) for batch in kombinationen_batches]
...
def check_keno_numbers_parallel(csv_path, kombinationen_df, start_date, end_date, batch_size=1000):
    ...
    results = []
    with ProcessPoolExecutor(max_workers=15) as executor:
        futures = [executor.submit(check_keno_parallel, batch, keno_data) for batch in kombinationen_batches]
...
def hauptprogramm():
    ...
    kombinationen_nach_archiv_df = vorpruefung_gegen_archiv_parallel(kombinationen_df, archiv_pfad, vorpruefung_datei_pfad)
    treffer_details = check_keno_numbers_parallel(keno_pfad, kombinationen_nach_archiv_df, '2021-01-01', '2021-12-31')
