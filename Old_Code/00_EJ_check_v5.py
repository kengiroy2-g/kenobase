...
def vorpruefung_gegen_archiv_parallel(kombinationen, archiv_pfad, ergebnis_datei_pfad, max_workers=4):
    if os.path.exists(ergebnis_datei_pfad):
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(hat_max_vier_gemeinsame_parallel, kombi, archiv_kombinationen): kombi for kombi in kombinationen}
            for future in tqdm(as_completed(futures), total=len(futures), desc="Vorprüfung gegen Archiv"):
                if future.result():
                    ergebnisse.append(futures[future])
...
def check_keno_numbers_parallel(csv_path, kombinationen_liste, start_date, end_date, max_workers=4):
    ...
    results = []
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Einreichen aller Aufgaben und Speichern der Futures in einer Liste
        futures = {executor.submit(check_keno_parallel, kombi, keno_data.copy()): kombi for kombi in kombinationen_liste}
        # Fortschrittsbalken für die Futures, die as_completed zurückgibt
